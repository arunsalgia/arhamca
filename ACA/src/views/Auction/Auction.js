...
let userCancelSold = false;  
function setUserCancelSold(newSts) { 
	userCancelSold = newSts;  
	//console.log("New calel ctstaus******************************   " + userCancelSold);
}
...
export default function Auction() {
      ...
      //console.log("I am " + localStorage.getItem("userName"));
        ...
        sockConn.on("auctionStatus", (newAuctionStatus) => {
          ...
          setAuctionStatus(newAuctionStatus);
				...
				sockConn.on("soldStart", (newAuctionStatus) => {
					console.log("Received sold start =============");
					setUserCancelSold(false);
					setBidPaused(true);
					setSoldDrawer("SOLD");
          console.log("Sold start");					
        });

				sockConn.on("soldCancel", (newAuctionStatus) => {
					console.log("Received calcel sold=============");
					setUserCancelSold(true);
					setBidPaused(false);
          //arunCancel = false;
					setSoldDrawer("");
          console.log("Cancel Sold");
        });
        ...
        sockConn.on("countDown", (message) => {
						//console.log("Countdown " + userCancelSold);
					  if (userCancelSold) return;
            console.log(message);
    ...
    async function sellplayer(count, tellOthers=false) {

			if (tellOthers) {
				console.log("Tell Others");
				try {
					await axios.get(`${process.env.REACT_APP_AXIOS_BASEPATH}/auction/soldstart/${localStorage.getItem("gid")}`);
        } catch (e) {
			...
			console.log(userCancelSold);
			
			// if cancel Sold then no action
			if (userCancelSold)  {     //if (arunCancel) { 
				count = 0; 
				setSoldDrawer("");
				//arunCancel = false;  
				setCountDown(0);
        ...
        try {
            ...
            setTimeout(() => sellplayer(count-1), 1000);
        } catch (e) {
            console.log("Unable to send countdown");
            console.log(e);
            setBidPaused(false);
...
*/
  ...
  function handleAccordionChange(panel) {
    ...
    return (
    <Grid align="center"m={1} p={1} container justifyContent="center" alignItems="center" >
    ...
    function ShowValueButtons() {
        ...
        if (auctionStatus === "RUNNING")
            ...
            {/*<div align="center">
                ...
                <Grid className={classes.bidButton} m={1} p={1} container justifyContent="center" alignItems="center" >
								<GridItem xs={12} sm={12} md={12} lg={12} >
                    <br />
                </GridItem>
    ...
    function ShowAuctionOverButton() {
			...
			{/*<Button variant="contained" color="primary" size="large" className={classes.button} onClick={handleAuctionOver}>
				...
				<VsButton name="Auction Over" align="center" onClick={handleAuctionOver} />
			</div>
			); 
    }
    ...
    function ShowAdminButtons() {
        ...
        if (localStorage.getItem("admin").toLowerCase() === "true")
            ...
            <GridItem xs={4} sm={4} md={4} lg={4} >
							...
							<VsButton name="Sold" disabled={((bidAmount === 0) || (bidPaused === true))} onClick={() => sellplayer(MAXCOUNTDOWN, true)} />
            </GridItem>
            <GridItem xs={4} sm={4} md={4} lg={4} >
							{/*<Button
									variant="contained"
...
/*
  ...
  function DisplayPendingButton() {
    ...
    ***/
		...
		async function sendCancelSold() {
			console.log("About to cancel");
			let myURL=`${process.env.REACT_APP_AXIOS_BASEPATH}/auction/soldcancel/${localStorage.getItem("gid")}`;
			console.log(myURL);
			await axios.get(myURL);
			console.log("Success");
		}
		
    //console.log(soldDrawer);
    return (
		<div align="center">
    ...
    {/*<Modal isOpen={soldDrawer == "SOLD"} onAfterOpen={afterOpenModal} onRequestClose={() => setSoldDrawer("")} style={modalStyles} contentLabel="Example Modal" ariaHideApp={false} >*/}
		<Modal isOpen={soldDrawer == "SOLD"}  style={modalStyles} contentLabel="Example Modal" ariaHideApp={false} >
      <DisplayCancel onCancel={sendCancelSold} />
      <ShowCountDown />
    </Modal>    
    <Modal
				isOpen={modalIsOpen}
