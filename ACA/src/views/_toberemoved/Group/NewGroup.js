...
import { getAllPrizeTable, getGroupCost } from 'views/functions';
...
var insufficientAmount = true;  function setInsufficientAmount(nowState) { insufficientAmount = nowState; }
...
export default function CreateGroup() {
  ...
  useEffect(() => {
    ...
    const a = async () => {
        ...
        setBalance(myBalance);			
				...
				setGroupCost(tmp);
				setInsufficientAmount( myBalance.wallet < tmp );
  ...
  const handleSubmit_new = async() => {
    ...
    try {
			let fee = await feebreakup(memberFee, balance.bonus);
			let myUrl = `${process.env.REACT_APP_AXIOS_BASEPATH}/group/createwithmaxplayers/${groupName}/${localStorage.getItem("uid")}/${bidAmount}/${selectedTournament}/${memberCount}/${memberFee}/${fee.bonus}/${maxPlayers}`;
			// console.log(myUrl);
  ...
  const handleConfirm = async() => {
    ...
    if (insufficientAmount) {
			sessionStorage.setItem("newGroupAmount", groupCost - balance.wallet);
			sessionStorage.setItem("newGroupInfo", JSON.stringify({bidAmount: bidAmount, tournament: selectedTournament, memberCount: memberCount, memberFee: memberFee, maxPlayers: maxPlayers }) );
    ...
    // groupName  bidAmount selectedTournament
		...
		let bonus = 0
    try {
      const response = await axios.get(`${process.env.REACT_APP_AXIOS_BASEPATH}/group/createwithmaxplayers/${groupName}/${localStorage.getItem("uid")}/${bidAmount}/${selectedTournament}/${memberCount}/${memberFee}/${bonus}/${maxPlayers}`);
      console.log(response.data);
      setNewGid(response.data.gid)      
      ...
      setCreateDisable(false);
      setRegisterStatus(err.response);
      console.log(`error code is ${err.response}`)
    }
  }
  ...
  return (
    ...
    <ValidatorForm className={gClasses.form} onSubmit={handleSubmit}>
      ...
      <TextValidator
        ...
        fullWidth      
        // size="small"  
        label="Number of Members in the group"
      ...
      <TextValidator
        ...
        fullWidth    
        // size="small"  
        label="Maximum Players Available for Auction"
        onChange={(event) => setMaxPlayers(event.target.value)}
        name="Maximum Players"
        type="number"
        validators={['required', 'minNumber:1', 'maxNumber:100', 'isNumeric']}
        errorMessages={['Member count to be provided', 'Minimum 1 player required', 'Maximum 100 players permitted',NOFRACTION]}
        value={maxPlayers}
      />
      <BlankArea/>
      <TextValidator
        ...
        fullWidth      
        // size="small"  
        label="Auction Coins available to member"
      ...
      <div align="center">
			...
			<VsButton name="Create" disabled={groupCode !== ""}  type="submit" />
    ...
    >
			...
			<DisplayBalance wallet={balance.wallet} bonus={balance.bonus}/>
			<BlankArea/>
      <Typography className={classes.new} align="center">Service Charge: {groupCost}</Typography>
      <Typography className={gClasses.error} align="center">
        {((insufficientAmount) ? "Insufficient amount in wallet." : "")}
      </Typography>
      <BlankArea />
			<VsButton align="center" name={insufficientAmount ? "Add to Wallet" : "Continue" } onClick={handleConfirm} />
      {/*<Button key="modalbutton" variant="contained" color="primary" size="medium"
        className={classes.dashButton} onClick={handleConfirm}>
        {((insufficientAmount) ? "Add to Wallet" : "Confirm Group Create")}
      </Button>*/}
    </Modal>
    </div>
