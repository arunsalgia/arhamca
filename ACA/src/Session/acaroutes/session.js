  ...
  setHeader(res);
	...
	var sessionInfo = await Session.aggregate(
		[ 
			{
			"$group" : { 
				"_id"   : { bid: "$bid", fid: "$fid", facultyName: "$facultyName", sidList: "$sidList", studentNameList: "$studentNameList" },
				sessionCount: { $sum: 1 }, 
				amount: { $sum: "$fees" }
				},
	...
	for(var i=0; i<sessionCountArray.length; ++i) {
		console.log(i);
		var tmp = studentArray.find(x => x.sid === sessionCountArray[i].sid);
		// console.log(tmp);
		if (tmp) { 
			console.log(tmp); 
			//sessionInfo[i].bid = tmp.bid; 
		}
		else   console.log(`Record not found for ${sessionCountArray[i].sid}`);
	}*/
  sendok(res, _.sortBy(sessionInfo, '_id.bid') ); 
})
...
router.get('/list/:month/:year', async function (req, res, next) {
  setHeader(res);
	...
	//sessionCountArray = [];
	for(var i=0; i < sessionInfo.length; ++i) {
		sessionInfo[i].bid = sessionInfo[i]._id.bid
...
router.get('/listbyfid/:fid', async function (req, res, next) {
  setHeader(res);
	
	var { fid } = req.params;
	
	//console.log("Faculty is ", fid); 
	//return sendok(res, "I am fine");
...
router.get('/listbybid/:bid', async function (req, res, next) {
  setHeader(res);
	...
	// First get list of students
	var allSessions = await Session.find({bid: bid}).sort({sessionNumber: -1});
