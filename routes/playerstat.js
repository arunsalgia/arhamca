...
async function ARUN_getTournameDetails(igroup) {
  var retVal = "";
  try {
...
async function ARUN_readDatabase(igroup) {
...
async function ARUN_statBrief(igroup, iwhichuser, doWhatSend)
{
  // Set collection name 
...
async function ARUN_statCalculation (igroup) {
  let myType = await getTournamentType(igroup);
...
async function unoptimised_update_cricapi_data_r3(logToResponse)
{
    let myindex;
...
async function update_cricapi_data_r1(logToResponse)
{
    ...
    for(aidx=0; aidx < matchesFromDB.length; ++aidx) {
      ...
      const cricData = await cricapi_get_score(mmm.cricMid)
      //if (cricData != null)
      if (cricData != null) {
        //var manofthematchPID = await updateMatchStats_roanuz(mmm, cricData);
        var manofthematchPID = await updateMatchStats_cricapi2023(mmm, cricData.score);
        // match over if man of the match declared 
        // OR
        // current time > matchEndTime
        let thisMatchOver = false;
        ...
        if (thisMatchOver) {
          ...
          // if tournament over
          // update tournament
          // update score and rank for all groups
...
// modified on 12th April 2023
async function update_cricapi_data_r2023(logToResponse)
{
    let myindex;
		console.log("S1 - 2023");
    ...
    var currtime = new Date(); 
    //let myfilter = { matchStartTime: {$lt: currtime }, matchEnded: false};
    let myfilter = { mid: 202304111930 };     // for testing purpose
    var matchesFromDB = await CricapiMatch.find(myfilter);
...
checkTournamentOver = async function (tournamentName) {
  ...
  console.log(matchesNotOver.length);
	
    // if no incomplete match then declare tournament as over
...
async function updateMatchStats_cricapi2023(mmm, cricdata)
{
  let briefIndex = -1;
  var currMatch = mmm.mid;
  let myType = mmm.type.toUpperCase();
  ...
  var manOfTheMatchPid = 0;  
  console.log(`-------------Match: ${currMatch} data update. Tournament; ${mmm.tournament}`)  
  return manOfTheMatchPid;
...
async function ARUN_processConnection(i) {
...
async function ARUN_sendDashboardData() {
...
/*
nodeSched.scheduleJob(SCHED_KEY, nextScheduleTime, async () => {
  // action to be performed
  //console.log("================================In Schedule==========================");
  //++cricTimer;    ---- Not to fetch data from CRICDATA. It will be done in APLSCORE
  ++clientUpdateCount;
  if (!db_connection) {
    return;
  }
  ...
  //console.log(arunSchedule, runningMatchCount)
  if ((arunSchedule > 0) || (runningMatchCount > 0)) {
    nextScheduleTime = new Date();
    nextScheduleTime.setSeconds(nextScheduleTime.getSeconds()+2);
    console.log(nextScheduleTime.toGMTString());
    console.log("rescheduling");
    schedJob.reschedule(nextScheduleTime);
  }
})

schedJob = nodeSched.scheduledJobs[SCHED_KEY];
//console.log(nodeSched);
*/


if (false) {
cron.schedule(crontmp, async () => {	
  if (arunSchedule > 0) 
    crontmp = `*/${arunSchedule}  * * * * *`;
  else
    crontmp = '0 */1 * * *';
  console.log(crontmp);
...
function timeToFetchMatches() {
	return false; //--------------arun arun. NO need to get match details. It will be done by APLSCORE
  var currtime = new Date();
