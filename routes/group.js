...
router.get('/getmaxplayers/:groupid', async function (req, res, next) {
setHeader(res); 

var { groupid } = req.params;
var groupRec = await IPLGroup.findOne({ gid: groupid });
if (!groupRec) { senderr(res,621, "Invalid Group"); return; }
sendok(res, {maxPlayers: groupRec.maxPlayers }); 
//sendok(res, {maxPlayers: 1 }); 
});

 
function sendDataToClient(groupid, player, balance) {
  let myList = _.filter(masterConnectionArray, x => x.gid == groupid && x.page === "AUCT");
  //console.log(myList);
	console.log(`Sending player change. 1st player ${player}.`);
	console.log(balance);
  myList.forEach(x => {
...
router.get('/getauctionstatus/:groupid', async function (req, res, next) {
  ...
  if (status === "RUNNING") {
		...
		//console.log(igroup, mytournament);
    if (mytournament.name !== SPECIAL_TOURNAMENT)
		if (mytournament.started) {
      console.log("Serring it ober---Cry cry cry");
			status = "OVER";
			gdoc.auctionStatus = status;
			await akshuUpdGroup(gdoc); 
		}
  }
  // find out how many persons in Auction page.
  
  sendok(res,status);
});

router.get('/getauctionstatuswithuid/:groupid/:uid', async function (req, res, next) {
  setHeader(res);

  var { groupid, uid } = req.params;
  let igroup = Number(groupid);
  uid = Number(uid);
  ...
  // find out how many persons in Auction page.
  //addToInAuctionPage(igroup, uid);
  let myCount = 1;  //countInAuctionPage(igroup);
  let totalCount = await GroupMemberCount(igroup); 
  sendok(res, {status: status, inPageCount: {in: myCount, total: totalCount} } );
});

router.get('/leavingauction/:groupid/:uid', async function (req, res, next) {
  setHeader(res);
  ...
  //delFromInAuctionPage(igroup, uid);
  sendok(res, "OK" );
});
...
router.get('/createwithmaxplayers/:groupName/:ownerid/:maxbid/:mytournament/:membercount/:memberFee/:bonusFee/:maxPlayers', async function (req, res, next) {
  
  setHeader(res);

  var { groupName, ownerid, maxbid, mytournament, membercount, memberFee, bonusFee, maxPlayers} = req.params;
  memberFee = Number(memberFee);
  bonusFee = Number(bonusFee);
	maxPlayers = Number(maxPlayers)
  console.log(memberFee, bonusFee);

  let memberfee = memberFee + bonusFee;
  mytournament = mytournament.toUpperCase();
  ...
  myRec.memberFee = memberfee;
  myRec.prizeCount = 1;
	// update maximum players permitted
	myRec.maxPlayers = maxPlayers;
	console.log(myRec);
	
	myRec.save();
  await akshuUpdGroup(myRec);

  // console.log(myRec._id);
	var myGroupCost = await getGroupCost();
  await WalletAccountGroupJoin(myRec.gid, myRec.owner, (myGroupCost), bonusFee);
  ...
  myGroupMemberRec.rank = 0;
  myGroupMemberRec.prize = 0;
  myGroupMemberRec.walletFee = memberFee;
  myGroupMemberRec.bonusFee = bonusFee;
  myGroupMemberRec.save();
...
router.get('/updatewithplayers/:groupId/:ownerId/:membercount/:maxPlayers', async function (req, res, next) {
  
  setHeader(res);

  let { groupId, ownerId, membercount, maxPlayers } = req.params;
  ...
  // new fields set default prize count as 1
  groupRec.memberCount = membercount;
  groupRec.maxPlayers = maxPlayers;
  //groupRec.memberFee = memberfee;
  // myRec.prizeCount = 1;
  groupRec.save();
  akshuUpdGroup(groupRec);
...
router.get('/join/:groupCode/:userid/:walletFee/:bonusFee', async function (req, res, next) {
  ...
  3) validate user has sufficient balance  -- not req. Will be done by client
 ...
 let groupRec;
  ...
  //console.log(groupRec);
	...
	// first check if user is already member of the group
  try {
		console.log(groupRec.gid, userid);
    let xxx = await GroupMember.findOne({gid: groupRec.gid, uid: userid})
		console.log(xxx);
    if (xxx) return senderr(res,617, "Already a member of the group");
    // Good!!!. User not a member of the group. We can add this user to the group
  } catch (err) {
		return senderr(res,621, "Unknown error");
  }
  ...
  let currentCount = await GroupMemberCount(groupRec.gid);
	//console.log(currentCount);
  if (currentCount >= groupRec.memberCount) { senderr(res,616, `Member count exceed limit`); return; }
  ...
  let total = await getGroupCost();
	//total += groupRec.memberFee;
  await WalletAccountGroupJoin(groupRec.gid, userRec.uid, total, bonusFee);
  ...
  myGroupMemberRec.rank = 0;
  myGroupMemberRec.prize = 0;
  myGroupMemberRec.walletFee = groupRec.memberFee;
  myGroupMemberRec.bonusFee = bonusFee;
  myGroupMemberRec.save();
...
router.get('/setfranchisename/:myUser/:myGroup/:myDisplayName', async function (req, res, next) {
  ...
  // first check if this name is used by some other member
	if (myDisplayName != "FRANCHISEENAMENOTWANTED") {
		gmRec = await GroupMember.findOne({gid: myGroup, displayName: myDisplayName});
		if (gmRec)
		if (gmRec.uid != myUser)
			return senderr(res, 625, "Duplicate Franchise name");
	}
	else
		myDisplayName = "";
	
  gmRec = await GroupMember.findOne({gid: myGroup, uid: myUser});
  if (gmRec) {
...
router.get('/gamestarted/:mygroup', async function (req, res, next) {
  ...
  //console.log(msg);
  if (SPECIAL_TOURNAMENT != "")  return sendok(res, "10000");
  sendok(res, msg);
});
...
router.get('/getgroupbycode/:groupcode', async function (req, res, next) {
	setHeader(res);

  var { groupcode } = req.params;
	let gdoc = await IPLGroup.findOne({_id: groupcode});
	if (gdoc)
		sendok(res, gdoc);
	else
		senderr(res, 601, "Invalid code");
});

	
	

router.get('/stat/:groupId', async function (req, res, next) {
  ...
  var { groupId } = req.params;
	...
	let allAuctions = await akshuGetAuction(groupId)
	let allPids = _.map(allAuctions, 'pid');
	let allTeams = _.map(allAuctions, o => _.pick(o, ['pid', 'team']));
	
	// now fetch scores of all players of this group. Get tournament name fro group record
	let gdoc = await akshuGetGroup(groupId);
	//console.log("Tournament: " + gdoc.tournament+BRIEFSUFFIX);
	let BriefStat = mongoose.model(gdoc.tournament+BRIEFSUFFIX, BriefStatSchema);
  let playerScores = await BriefStat.find({ sid: {$lte: 0 }, pid: {$in: allPids} });		//.sort({playerName: 1});
	//for(var ars=0; ars<playerScores.length; ++ars) {
	//	console.log(playerScores[ars].manOfTheMatch);
	//}
	//console.log(playerScores[0]);
	
	// now check the scores for Maximum run and add to record where sid is 0
	let mymaxrunwkt = playerScores.find(x => x.sid === MaxRunMid)
	if (mymaxrunwkt) {
		var maxplayer = playerScores.find( x => x.sid === 0  && x.pid === mymaxrunwkt.pid);
		//console.log(maxplayer);
		if (maxplayer) {
			maxplayer.score += mymaxrunwkt.score;
			maxplayer.maxTouramentRun = mymaxrunwkt.maxTouramentRun;
		}
	}
	
	// now check the scores for Max wicket and add to record where sid is 0
	mymaxrunwkt = playerScores.find(x => x.sid === MaxWicketMid)
	if (mymaxrunwkt) {
		var maxplayer = playerScores.find( x => x.sid === 0  && x.pid === mymaxrunwkt.pid);
		if (maxplayer) {
			maxplayer.score += mymaxrunwkt.score;
			maxplayer.maxTouramentWicket = mymaxrunwkt.maxTouramentWicket;
		}
		//console.log(maxplayer);
	}
	
	//console.log(mymaxrunwkt);
	//console.log(maxplayer);
	
	// now remove these 2 record of max run and max wicket. No more required
	playerScores = playerScores.filter(x => x.sid === 0);
	//for(var ars=0; ars<playerScores.length; ++ars) {
	//	console.log(playerScores[ars].manOfTheMatch);
	//}
	// Add multiplying factor for captions and vice captains. Add bonus 
	let allCaptains = await Captain.find({gid: groupId}, {captain: 1, viceCaptain: 1});
	allCaptains.forEach ( capRec => {
		let tmpRec = playerScores.find(x => x.pid === capRec.captain);
		if (tmpRec) {
			tmpRec.score += tmpRec.run*BonusRun["T20"]*(Captain_MultiplyingFactor-1) + tmpRec.wicket*BonusWkt["T20"]*(Captain_MultiplyingFactor-1)
			tmpRec.playerName += "-C";
			//console.log(tmpRec);
		}
		tmpRec = playerScores.find(x => x.pid === capRec.viceCaptain);
		if (tmpRec) {
			//console.log(tmpRec);
			tmpRec.score += tmpRec.run*BonusRun["T20"]*(ViceCaptain_MultiplyingFactor-1) + tmpRec.wicket*BonusWkt["T20"]*(ViceCaptain_MultiplyingFactor-1)
			tmpRec.playerName += "-VC";
		}
	});
	...
	//	let tmp = allAuctions.find(x => x.pid === ps.pid);
	//	ps["team"] = tmp.team;
	//	console.log(ps);
	//});
	
	// update bid amount 
	allAuctions.forEach( p => {
		let tmpRec = playerScores.find(x => x.pid === p.pid);
		if (tmpRec) tmpRec.playerName += " (Bid: " + p.bidAmount + ")";
	});
	
	let allGroupMembers = await akshuGetGroupMembers(groupId);
	allGroupMembers =  _.map(allGroupMembers, o => _.pick(o, ['uid', 'displayName']));
	allGroupMembers.forEach( gm => {
		gm["pidList"] = _.map(allAuctions.filter(x => x.uid === gm.uid), 'pid');
		// now also provide the team in pislist
		//gm["pidList"] = _.map(allAuctions.filter(x => x.uid === gm.uid), o => _.pick(o, ['pid', 'team']));
		gm["totalScore"] = _.sumBy(playerScores.filter(x => gm["pidList"].includes(x.pid)), 'score');
		//console.log(gm);
	});
	allGroupMembers = _.sortBy(allGroupMembers, 'totalScore').reverse();

	
	// send back details
	sendok(res, {members: allGroupMembers, stat: playerScores, teams: allTeams} )
});




router.get('/statcheck/:groupId', async function (req, res, next) {
