...
router.get('/groupfee', async function (req, res, next) {
  let groupCost = await getMaster("GROUPCOST");
	groupCost = parseInt(groupCost);
  sendok(res, {wallet: groupCost, bonus: 0});
});
...
router.get('/adduidlist/:uidListStr/:amount', async function (req, res, next) {
  setHeader(res);
  var {uidListStr, amount} = req.params;
	
	console.log(uidListStr, amount);
	// find out if this is the first recharge by user
	//let count = await rechargeCount(userId);
  let count = 5;
	
	var uidList = uidListStr.split(",");
	console.log(uidList);
	for(var i=0; i<uidList.length; ++i) {
		var myUser = await akshuGetUser(Number(uidList[i])); 
		console.log(myUser);
			
		let myTrans = createWalletTransaction();
		myTrans.transType = WalletTransType.refill;
		myTrans.uid = myUser.uid;
		myTrans.transSubType = "ByAdmin";
		myTrans.amount = amount;
		await myTrans.save();
	}
  sendok(res, {count: uidList.length});
