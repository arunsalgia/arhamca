...
function setAllInterested(myGid) {
  for(var i=0; i<masterConnectionArray.length; ++i) {
    if ((masterConnectionArray[i].page === "AUCT") && (masterConnectionArray[i].gid == myGid))
      masterConnectionArray[i].interested = true;
  }
}
...
router.get('/notinterested/:igroup/:iuser/:newState', async function(req, res, next) {
  // AuctionRes = res;
  setHeader(res);
  var {igroup,iuser,newState}=req.params;

  igroup = parseInt(igroup);
  iuser = parseInt(iuser);
  newState = (newState === 'true');
  
  let tmp = masterConnectionArray.find(x => x.page === "AUCT" && x.gid === igroup && x.uid === iuser);
  if (tmp)  tmp.interested = newState;
...
function sendPlayerChangeToClient(groupId, pData, bData) {
  var myList = _.filter(masterConnectionArray, x => x.gid == groupId && x.page === "AUCT");
  //console.log(pData);
  myList.forEach(x => {
...
function sendBidOverToClient(bidData) {
  var myList = _.filter(masterConnectionArray, x => x.gid == bidData.gid && x.page === "AUCT");
  // console.log(myList);
  myList.forEach(x => {
...
function sendNewBidToClient(groupRec) {
	...
	//console.log(groupRec.gid);
  var myList = _.filter(masterConnectionArray, x => x.gid == groupRec.gid && x.page === "AUCT");
  //console.log(myList);
  myList.forEach(x => {
...
function sendSoldStartToClient(groupRec) {
  var myList = _.filter(masterConnectionArray, x => x.gid == groupRec.gid && x.page === "AUCT");
  //console.log(myList);
  myList.forEach(x => {
	  console.log("Sending message to", x.gid, x.uid);
    io.to(x.socketId).emit('soldStart', {countDown: 0});
  });
	console.log("soldStart All DOne");
}

function sendSoldCancelToClient(groupRec) {
  var myList = _.filter(masterConnectionArray, x => x.gid == groupRec.gid && x.page === "AUCT");
  //console.log(myList);
  myList.forEach(x => {
	  console.log("Sending message to", x.gid, x.uid);
    io.to(x.socketId).emit('soldCancel', {countDown: 0});
  });
	console.log("soldCancel All DOne");
}
...
router.get('/soldstart/:groupId', async function (req, res, next) {
	setHeader(res);
	var {groupId }=req.params;
	console.log("======================**********************=============== Revcived sold start for gid " + groupId);
  groupId = Number(groupId);
	let myGroup = await akshuGetGroup(groupId);
  sendSoldStartToClient(myGroup);
  sendok(res, "Done");
});

router.get('/soldcancel/:groupId', async function (req, res, next) {
	setHeader(res);
	var {groupId }=req.params;
	console.log("======================**********************=============== Revcived sold cancel for gid " + groupId);
  groupId = Number(groupId);
	let myGroup = await akshuGetGroup(groupId);
  sendSoldCancelToClient(myGroup);
  sendok(res, "Done");
});
...
router.get('/nextbid/:groupId/:userId/:playerId/:bidAmount', async function(req, res, next) {
  ...
  if (groupRec.currentBidUid == userId) { senderr(res, 711, `User bid already registered`); return; }  
...
router.get('/status/:groupId', async function(req, res, next) {
  // AuctionRes = res;
  setHeader(res);
  var {groupId}=req.params;
