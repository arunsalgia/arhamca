...
router.get('/arun/:tournamentName', async function(req, res, next) {
  ...
  var {tournamentName} = req.params;
  console.log("Arun called");
	
  let myTournament = await Tournament.findOne({name: tournamentName})
  if (!myTournament) { senderr(res, 601, "Invalid tournament"); return; };
  await checkTournamentOver(tournamentName);
  sendok(res, "Checking done");
});

router.get('/setstarted/:tournamentName', async function(req, res, next) {
  // TournamentRes = res;
  setHeader(res);
  if (!db_connection) { senderr(res, DBERROR, ERR_NODB); return; }

  var {tournamentName} = req.params;
  console.log("TOurnament started called");
	tournamentName = tournamentName.toUpperCase();
	await updateTournamentStarted(tournamentName);
  sendok(res, "Done");
});
...
async function publishTournament(res, filter_tournament)
{
  ...
  if (SPECIAL_TOURNAMENT != "") {
    var specialTour = await Tournament.findOne({name: SPECIAL_TOURNAMENT});
    tlist.push(specialTour);
  }
